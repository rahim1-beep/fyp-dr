...
**R3 — The test set is touched once.** Model selection, threshold tuning, early stopping, and every hyperparameter decision use validation only. Test metrics are computed at the end of each phase and recorded. If you find yourself tuning against test numbers, stop and say so.
